<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>申请班级</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0,user-scalable=no">
  <link rel="stylesheet" href="http://cdn.bootcss.com/bootstrap/3.3.5/css/bootstrap.min.css">
  <link rel="stylesheet" href="/css/huangpu_index.css">
</head>
<body>
<div class="container">
  <br>
  <form action="" class="form-horizontal">
    <div class="form-group">
      <label class="form-label col-xs-4">班级名称</label>
      <div class="col-xs-8">
        <input type="text" class="form-control" name="classname">
      </div>
    </div>
    <hr style="">
    <div class="form-group">
      <label class="form-label col-xs-4">班级规模</label>
      <div class="col-xs-8">
        <select class="form-control" name="classtype">
          <option value="selected">0-10人</option>
          <option>10-30人</option>
          <option>30-80人</option>
          <option>80-200人</option>
          <option>>200人</option>
        </select>
      </div>
    </div>
    <hr>
    <div class="form-group">
      <label class="form-label col-xs-4">班级形式</label>
      <div class="col-xs-8">
        <select class="form-control" name="classtype">
          <option value="selected">模式1</option>
          <option>模式2</option>
          <option>模式3</option>
          <option>模式4</option>
          <option>模式5</option>
        </select>
      </div>
    </div>
    <hr>
    <div class="form-group">
      <label class="form-label col-xs-4">招募时间</label>
      <div class="col-xs-8">
        <input type="date" class="form-control inputdate" name="classtime">
      </div>
    </div>
    <hr>
    <div><label class="form-label">班级导师</label><span class="float-right">点击添加导师</span></div>
    <div>
      <div class="media-left media-middle">
        <img class="docpic" src="/image/icons/private.png" alt="...">
      </div>
      <div class="media-body">
        <br>
        <h4 class="media-heading">某某某&nbsp;
          <small>主治医师</small>
        </h4>
        <p>中南医院呼吸科</p>
      </div>
    </div>
    <div>
      <div class="media-left media-middle">
        <img class="docpic" src="/image/icons/private.png" alt="...">
      </div>
      <div class="media-body">
        <br>
        <h4 class="media-heading">某某某&nbsp;
          <small>主治医师</small>
        </h4>
        <p>中南医院呼吸科</p>
      </div>
    </div>
    <hr>
    <div><label class="form-label">班级助教</label><span class="float-right">点击添加助教</span></div>
    <div>
      <div class="media-left media-middle">
        <img class="docpic" src="/image/icons/private.png" alt="...">
      </div>
      <div class="media-body">
        <br>
        <h4 class="media-heading">某某某&nbsp;
          <small>主治医师</small>
        </h4>
        <p>中南医院呼吸科</p>
      </div>
    </div>
    <hr>
  </form>
  <button class="btn btn-block btn-primary btn-lg">提交申请</button>
  <br>
</div>


<script src="https://cdn.bootcss.com/jquery/2.1.4/jquery.min.js"></script>
<script src="http://cdn.bootcss.com/bootstrap/3.3.5/js/bootstrap.min.js"></script>
</body>
</html>